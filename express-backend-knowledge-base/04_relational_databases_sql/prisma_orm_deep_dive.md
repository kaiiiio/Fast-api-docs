...
**Prisma Client:** An auto-generated, type-safe database client created from your Prisma schema. It provides methods like `findMany()`, `create()`, `update()` with full TypeScript support and auto-completion.

**Prisma Schema:** A declarative file (`schema.prisma`) that defines your database structure, models, and relationships. It's the single source of truth for your database structure.

**Prisma Migrate:** A migration system that generates SQL migrations from your Prisma schema changes. It tracks schema versions and applies changes to your database systematically.

**Prisma Studio:** A visual database browser that lets you view and edit data in your database through a GUI. Useful for development and debugging.

**Model:** A definition in the Prisma schema that represents a database table. Each model maps to a table and defines its columns, types, and relationships.

**Relation:** A connection between two models in Prisma schema (e.g., User has many Posts). Prisma handles foreign keys and provides type-safe relation queries.

**Migration:** A version-controlled SQL file generated by Prisma Migrate that contains database schema changes. Ensures consistent database structure across environments.

**$disconnect():** A Prisma Client method that closes all database connections in the connection pool. Essential for graceful application shutdown to prevent connection leaks.

**Aggregate:** Prisma operations that perform statistical calculations on database columns (e.g., `_count`, `_avg`, `_sum`, `_min`, `_max`). Used for analytics and reporting.

**Transaction:** A Prisma feature (`$transaction()`) that executes multiple database operations atomically - either all succeed or all fail together. Ensures data consistency.

**Include:** A Prisma query option that loads related data (e.g., `include: { posts: true }`). Similar to SQL JOINs but type-safe.

**Select:** A Prisma query option that specifies which fields to return. Optimizes queries by fetching only needed data.

**Upsert:** A Prisma operation that updates a record if it exists, or creates it if it doesn't. Combines update and insert in one atomic operation.
...
// beforeExit event - Triggered when Node.js empties its event loop
// $disconnect() - Closes all database connections in the connection pool
// This prevents "connection pool exhausted" errors and ensures clean shutdown
// IMPORTANT: Always disconnect Prisma Client when your app shuts down
...
// Aggregate - Performs statistical calculations on database columns
// _count - Counts the number of records (e.g., total posts)
// _avg - Calculates average value of a numeric field (e.g., average views)
// _max - Finds maximum value in a field (e.g., most viewed post)
// _min - Finds minimum value in a field (e.g., least viewed post)
// _sum - Calculates sum of all values in a field (e.g., total views across all posts)
// Returns: { _count: { id: 100 }, _avg: { views: 250.5 }, _max: { views: 1000 }, _min: { views: 10 }, _sum: { views: 25050 } }
