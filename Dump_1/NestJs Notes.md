...
# ================================================================
> [!TIP]
> **Confused by the 10,000+ lines?** Follow the **[Master Full-Stack Preparation Flow](file:///c:/Users/nikit/Downloads/Nikita/Prepare/Knowledge-base/Dump_1/Master_FullStack_Interview_Guide.md)** for a structured learning path!
...
On-demand revalidation allows you to update cached data immediately when a change occurs, rather than waiting for an Revalidation timer (ISR). You can manually trigger Next.js to purge the specific cache.
...
**What it is:**  
ARIA (Accessible Rich Internet Applications) is a set of attributes that define ways to make web content and web applications more accessible to people with disabilities. It bridges the gap when standard HTML tags (like `<div>` or `<span>`) are used to create complex interactive widgets.

**Core Principles:**  
1. **ARIA is a last resort**: Use semantic HTML (like `<button>`, `<nav>`, `<input>`) whenever possible. ARIA doesn't change behavior; it only changes how screen readers *perceive* the element.
2. **Don’t break semantics**: Do not add ARIA roles to elements that already have a specific meaning (e.g., don't add `role="button"` to an `<a>` tag).

**Common Attributes Explained:**  
- `role`: Defines the specific "type" of component (e.g., `menu`, `dialog`, `tablist`).
- `aria-expanded`: A boolean state (`true`/`false`) that tells screen readers if a collapsible section (like a dropdown) is currently open.
- `aria-controls`: Links the trigger element to the ID of the element it controls.
- `aria-haspopup`: Signals that clicking the element will trigger a sub-menu or a floating window.
- `aria-label`: Provides a string label for elements that don't have visible text (like an icon button).

**Practical Example (Interactive Dropdown):**
```tsx
'use client';
  ...
  return (
    <div className="relative">
      <button
        ...
        // Critical for screen readers to know state
        aria-expanded={isOpen}
        // Link to the content ID
        aria-controls="main-dropdown"
        // Explain that a menu will appear
        aria-haspopup="true"
...
**Interview Insight:**  
"If an interviewer asks about accessibility, mention the **First Rule of ARIA**: *'If you can use a native HTML element or attribute with the semantics and behavior you require already built-in, instead of re-purposing an element and adding an ARIA role, state or property to make it accessible, then do so.'* Only use ARIA when custom behavior (like a complex drag-and-drop or a custom-styled toggle) makes native elements insufficient."
...
### Conceptual Overview
- **Strategy**: SWR follows a "Stale-While-Revalidate" approach: it first serves stale (cached) data, then revalidates it by fetching fresh data in the background, finally updating the UI once the fresh data is available.
- **Focus Revalidation**: SWR automatically revalidates data when the window regains focus, ensuring users always see the most up-to-date information.
- **Optimistic UI (Mutate)**: Use optimistic updates to show the results of an action immediately while the server request is pending. SWR automatically rolls back to the previous state if the server request fails.
...
- `Async/await` provides cleaner and more readable syntax while performing asynchronous operations using `Promises` under the hood.
- For optimal performance, always use `Promise.all` when executing multiple independent API calls in parallel.
